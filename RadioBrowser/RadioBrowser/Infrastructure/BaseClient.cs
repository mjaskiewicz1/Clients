...
public class BaseClient(IRestClient client)
{
    ...
    protected async Task<T> RequestAsync<T>(RestRequest request) where T : class
    {
        ...
        if (string.IsNullOrWhiteSpace(res.Content))
            throw new RadioBrowserException($"[{request.Resource} => Status: {res.StatusCode}] Received empty response!", res.StatusCode);

        try
        {
            var model = JsonSerializer.Deserialize<T>(res.Content, options: DeserializeSettings) ??
                        throw new RadioBrowserException($"Failed to deserialize [{request.Resource}] response! Content parsed to null.", res.StatusCode);
        ...
        {
            throw new RadioBrowserException($"Failed to deserialize [{request.Resource}] response due to invalid JSON format!", ex);
        }
    }
}